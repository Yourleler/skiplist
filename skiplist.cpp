...
template <class K, class E>
Strictskiplist<K, E>::Strictskiplist(vector<pair<K, E>>arr, K maxkey, int maxnum, double prob)
{
    cutoff = prob;
    maxlevel = static_cast<int>(log(maxnum + 1) / log(2)) + 1; //严格跳表每层是下一层的1/2，所以层高应为log_2(n+1)+1取整  以m为底：log(n)/log(m)
    levels = 0;
...
int main()
{
    ...
    Strictskiplist<int, int>ss(arr, m, m);
    //for (int i = 0; i < n; i++)
    //{
    //    int a;
    //    cin >> a;
...
//        sort(arr.begin() + 1, arr.end()); // 排为正序
//
//        Strictskiplist<int, int> ss(arr, m, m); // 随机生成n个数，初始化为严格跳表
...
//    outputFile << 5000 << " " << 10000 << " ";//1000个数，最大数1000   接着是30000个数对a b，a是operationType0为插入，1为删除，2为查找，b是操作数，对应num
